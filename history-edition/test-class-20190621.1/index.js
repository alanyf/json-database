...
class Table{
    constructor(param){
        this.instanceParamCheck();// 实例话入参检查
        // 公有属性
    ...
    static instanceParamCheck(param){
        if(typeOf(param) !== 'object'){
            throw new Error('The param must be a Object!');
        }
        if(!(typeOf(param.name) === 'string' && 
           typeOf(param.path) === 'string' &&
           typeOf(param.data) === 'array' &&
...
class Database{
    constructor(){
        // 公有属性
        this.tables = {};
        this._DATA_PATH = __dirname + '/.data';
        this._TABLES_PATH = this._DATA_PATH + '/tables.json';
        this._DB_CONNECTED_FLAG = false;
    ...
    static saveTableSync(_path, _tableObj){
        const str = JSON.stringify(_tableObj);
        fs.writeFileSync(_path, str);
        return _path + " table save successfully!";
    }
    static createDataFolder(){
        if(!fs.existsSync(this._DATA_PATH)){
            fs.mkdirSync(this._DATA_PATH, { mode: '0777' })
        }
    }
    static readTableSync(_path){
    ...
    connectDatabase(){
        if(this._DB_CONNECTED_FLAG === true){
            return;
        }
        this.createDataFolder();// 如果存放数据的文件夹不存在就创建一个
        const tablesObj = readTableSync(_TABLES_PATH);
        if(tablesObj){
            this.tables = tablesObj;
        }
        this._DB_CONNECTED_FLAG = true;
    }
    addTable(tableName, template){
        ...
        const tablePath = this._DATA_PATH + '/' + tableName + '.json';
        const tableInfObj = {
            _id: Object.keys(this.tables).length||0,
            name: tableName,
            path: tablePath,
            createTime: Date.now()
        };
        this.tables[tableName] = tableInfObj;
        const tableObj = {
            name: tableName,
            path: tablePath,
            data: [],
        ...
        saveTableSync(_TABLES_PATH, this.tables);

        return tableInstance;
    }
    getTable(tableName){
        const tableInf = this.tables[tableName];
        if(tableInf){
